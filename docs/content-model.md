...
## Поле `place` («Место») у тура — добавлено 2026-08-20

Карточка тура в блоке КАЛЕНДАРЬ в макете двухстрочная: сверху мелким капсом
**название тура** (`name`), под ним крупным Cormorant — **место** (`place`).

| Шаг | Файл | Что |
|---|---|---|
| 1. Схема | [`sanity/schemas/tour.ts`](../sanity/schemas/tour.ts) | поле `place`, тип `string`, необязательное |
| 2. Запрос | [`lib/sanity.queries.ts`](../lib/sanity.queries.ts) | `place` в `toursQuery` |
| 3. Рендер | [`components/sections/tour-calendar-card.tsx`](../components/sections/tour-calendar-card.tsx) | если `place` заполнен — он идёт крупной строкой, а название тура мелкой; если пуст, карточка выглядит как раньше |
