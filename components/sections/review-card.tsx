...
export function ReviewCard({ review }: ReviewCardProps) {
  return (
    <div
      data-review-card
      className="group relative flex h-full min-w-0 w-full max-w-full flex-col rounded-2xl border border-[#e5e0db] bg-white px-4 pt-3 pb-3 shadow-card transition-all duration-500 hover:border-[#bea692]/30 hover:shadow-card-hover md:px-5 md:pt-4 md:pb-4"
    >
      ...
      <div className="absolute top-3 -left-2 h-16 w-1 rounded-full bg-gradient-to-b from-[#bea692] to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />

      {/* Шапка: высота по ряду синхронизируется ReviewsGridRowAlign (сетка и карусель) */}
      <div
        data-review-header
        className="relative z-10 flex min-h-0 min-w-0 flex-col justify-start border-b border-[#e5e0db] pb-3 box-border"
      >
