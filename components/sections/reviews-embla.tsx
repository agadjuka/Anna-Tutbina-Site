      ...
      cn(
        ...
        isFull
          ? [
              "flex-[0_0_85%]",
              "sm:flex-[0_0_82%]",
              /* Ровно четыре карточки в ряд и ни одной обрезанной — просьба
                 заказчика. Порог опущен с `xl` (1280) до `lg` (1024) по правке
                 2026-08-21: «сделай их уже, чтобы 4 входили от 1024». Раньше
                 между 1024 и 1280 в кадре было три — заказчик открывал сайт в
                 окне 1173px и видел три вместо четырёх.

                 Промежуточная ступень в три карточки убрана совсем: она давала
                 карточку ШИРЕ (350px на 1173), чем четыре на 1280 (281px), и
                 ряд «скакал» в ширине при изменении окна. Теперь 2 → 4.
                 На 1024 карточка получается ~222px — узко, но заказчик выбрал
                 это осознанно. Зазор 44px из макета сам ужимается через `vw`,
                 иначе на 1024 три зазора съели бы 132px. */
              "md:flex-[0_0_calc((100%-min(2.29vw,44px))/2)]",
              "lg:flex-[0_0_calc((100%-3*min(2.29vw,44px))/4)]",
            ]
          : "flex-[0_0_82%]"
      ),
    [isFull]
