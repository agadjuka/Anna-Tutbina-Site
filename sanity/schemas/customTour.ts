...
const customTour = defineType({
  ...
  fields: [
    ...
    defineField({
      name: 'decorPhoto',
      title: 'Цветок-декор в блоке «Сотрудничество»',
      description:
        'Полупрозрачный декор за текстом в блоке «Сотрудничество» на главной. ' +
        'PNG с прозрачным фоном. Раньше сюда подставлялось второе фото из блока ' +
        '«О проекте» — из-за этого один и тот же цветок стоял в обоих блоках, ' +
        'хотя в макете они разные. Оставьте поле пустым, чтобы вернуть старое ' +
        'поведение.',
      type: 'image',
      options: {
        hotspot: true,
