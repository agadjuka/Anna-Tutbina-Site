...
components/home/         # боевая главная: состав секций + включение CSS-масштаба
...
- **⚠️ Размеры на главной — ЗАКОН ОДНОГО МНОЖИТЕЛЯ (с 2026-08-25).** Каждый размер в
  lg-ветке — кегль, отступ, зазор, ширина колонки, `min-height` — записан как
  `calc(<число> * var(--ona-u))`, где `<число>` — величина в пикселях **при ширине окна
  1280** (вид, который согласовал заказчик), а `--ona-u = min(100vw / 1280, 1.5px)`.
  Раз всё умножается на одно и то же число, страница на 1440/1600/1920 — **точная копия**
  себя же на 1280: сколько строк занимал заголовок, столько и занимает.
  **Ошибка — любая своя формула:** `clamp(12px, …vw, …)`, `max(11px, …)`, голый
  `lg:text-[17px]`, процент от контейнера. Пол в `clamp` кажется безобидным, но именно
  из-за него кегль переставал уменьшаться, колонка продолжала — и перенос уезжал
  (заголовок FOUNDERS шёл в 3 строки на 1920 и в 2 на 1280).
  Слой целиком — секция «Облегчённый масштаб главной» в конце `app/globals.css`,
  разбор и таблицы — [`docs/redesign/lightweight-scale-plan.md`](docs/redesign/lightweight-scale-plan.md).
- **Проверять правки на 1280 в первую очередь.** Заказчик смотрит сайт именно на этой
  ширине; половина его замечаний на 1920 не воспроизводилась вовсе.
- **После любой правки размеров прогнать аудит устойчивости** (`audit.js`, см.
  lightweight-scale-plan.md §6): он открывает главную на 1024/1280/1366/1440/1600/1920/2560
  и показывает элементы, у которых поменялось число строк. **Норма — пустой список.**
